...
app.post("/sign", async (req, res) => {
  ...
  try {
    ...
    if (!user)
      return res
        .status(404)
        .json({ error: "Usuário não encontrado. Login ou senha incorretos." });
    ...
    if (user) {
      ...
      User.create({
        ...
        .then((createdUser) => {
          ...
          res
            .status(200)
            .json({ msg: "Usuário cadastrado", createdUser, token });
        })
        .catch((error) => {
    ...
    .then((book) => {
      if (!book) {
        Book.create({
          ...
          .then(() => {
            res.status(200).json({ msg: "Livro adicionado à estante." });
          })
          .catch((error) => {
    ...
    .then((book) => {
      if (book) {
        Book.destroy({
          ...
          .then(() => {
            res.status(200).json({ msg: "Livro removido da estante." });
          })
          .catch((error) => {
            console.log(error);
...
app.put("/favoriteBooks", (req, res) => {
  try {
    ...
    Book.update(
      { favorite: true },
      { where: { title: title, login: storageLogin, apiID: bookId } }
    );
...
app.put("/removeFavorite", (req, res) => {
  try {
    ...
    const updatedBookCount = Book.count({
      where: { title: title, favorite: false, login: storageLogin },
    });

    if (updatedBookCount === 0) {
      return res
        .status(404)
        .send({ msg: "Livro não encontrado na lista de favoritos." });
    }
...
app.put("/readBooks", async (req, res) => {
  try {
    ...
    await Book.update(
      { read: true, toRead: false },
      { where: { title: title, login: storageLogin, apiID: bookId } }
    );

    const updatedBookCount = await Book.count({
      where: { title: title, read: true, login: storageLogin },
    });

    if (updatedBookCount === 0) {
      return res
        .status(404)
        .send({ msg: "Livro não encontrado na lista de leitura." });
    }
...
app.put("/toReadBooks", async (req, res) => {
  try {
    ...
    if (!bookItem || !bookItem.title) {
      return res
        .status(400)
        .send({ msg: "O livro não foi fornecido corretamente." });
    }

    await Book.update(
      { toRead: true, read: false },
      { where: { title: bookItem.title } }
    );

    const updatedBookCount = await Book.count({
      where: { title: bookItem.title, toRead: true },
    });
    if (updatedBookCount === 0) {
      return res
        .status(404)
    ...
    res
      .status(200)
      .send({ msg: "Livro adicionado à lista de livros Não Lidos!" });
  } catch (error) {
    console.error(error);
    res.status(500).send({ msg: "Erro ao atualizar o livro." });
  }
